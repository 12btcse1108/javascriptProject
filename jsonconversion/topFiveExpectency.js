...
function jsonValue(countrycode, totalexp) {
...
rl.on('line', function(value) //reading of file started
    {
        ...
        if (count == 0) {
            var header = value.split(','); //splitting of headers
        } else {
            var first_row = value.match(/(".*?"|[^",\s]+)(?=\s*,|\s*$)/g); //first row reading starts
...
rl.on('close', function() { //reading file end
    for (var key in mapTotalCount) {
        var json1 = new jsonValue(key, mapTotalCount[key])
        finalValue.push(json1); //final array
    }
    file.writeFile('../json/jsonbar.json', JSON.stringify(finalValue), 'utf-8'); //conversion from csv to json
})
